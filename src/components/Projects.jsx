...
import { ExternalLink, Github, Trophy, X } from 'lucide-react';
...
export default function Projects() {
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => requestAnimationFrame(() => setModalVisible(true)));
    return () => {
  ...
  return (
    <section id="projects" className="relative w-full bg-black py-8 text-white">
      {selectedProject && (
        <div
          className={`fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 transition-opacity duration-300 ${modalVisible ? 'opacity-100' : 'opacity-0'}`}
        ...
        >
          <div
            className={`relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-xl border border-white/10 bg-black shadow-xl transition-all duration-300 ease-out ${modalVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => setSelectedProject(null)}
              className="absolute right-3 top-3 z-10 rounded-md p-1.5 text-white/60 hover:bg-white/10 hover:text-white transition-colors"
              aria-label="Close"
            >
              <X size={20} />
            </button>
            <div className="relative aspect-video overflow-hidden">
              {selectedProject.video ? (
                ...
                <div className="flex h-full min-h-[200px] items-center justify-center bg-white/5 text-white/40 text-sm font-medium">
                  Coming soon
                </div>
              )}
            ...
            <div className="p-6">
              <h2 id="modal-title" className="text-xl font-semibold">{selectedProject.title}</h2>
              {selectedProject.dateRange && (
                <p className="mt-1 text-xs text-white/50">{selectedProject.dateRange}</p>
              )}
              ...
              <p className="mt-3 text-sm text-white/70">{selectedProject.description}</p>
              ...
              <div className="mt-4 flex items-center gap-2">
                {selectedProject.link && (
                  <a
                    ...
                    className="inline-flex items-center gap-2 rounded-md border border-white/15 bg-white/5 px-3 py-2 text-sm text-white/80 hover:bg-white/10 transition-colors"
                  >
          ...
          {[...projects, { placeholder: true, id: 'placeholder-1' }, { placeholder: true, id: 'placeholder-2' }].map((item) => {
            ...
            return (
              <div
                ...
                className="group rounded-xl overflow-hidden border border-white/10 bg-white/5 hover:bg-white/10 transition-colors flex flex-col cursor-pointer"
              >
